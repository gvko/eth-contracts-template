...
const provider = new providers.JsonRpcProvider(ETHEREUM_NODE_API_URL)
const wallet = new Wallet(WALLET_PRIVATE_KEY, provider)
const contract = new Contract(CONTRACT_ADDRESS_ETH, contractABI.abi, wallet)
...
async function mintNft(tokenMetadataUrl, toAddress = WALLET_PUBLIC_KEY) {
  ...
  try {
    ...
    console.log(`SEE THE TX AT: https://goerli.etherscan.io/tx/${tx.hash}`)
  } catch (err) {
